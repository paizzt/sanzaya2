...
export default function UcEdit({ ucRequest, users, vehicles, isAdmin }) {
    ...
    const { data, setData, put, processing, errors, reset } = useForm({
        user_id: ucRequest.user_id || user.id,
        entity: ucRequest.entity || 'PT. Sanzaya Medika Pratama',
        department: ucRequest.department || 'Sales',
        destination_city: ucRequest.destination_city || '',
        departure_date: ucRequest.departure_date ? ucRequest.departure_date.substring(0,10) : '',
        return_date: ucRequest.return_date ? ucRequest.return_date.substring(0,10) : '',
        estimated_days: ucRequest.estimated_days || 0,
        companions: ucRequest.companions || [],
        transportation_type: ucRequest.transport_type || 'Darat',
        vehicle_number: ucRequest.vehicle_number || '',
        items: ucRequest.items && ucRequest.items.length > 0 
                ? ucRequest.items.map(i => ({ item_name: i.item_name, quantity: i.quantity, estimated_cost: i.estimated_cost })) 
                : [{ item_name: '', quantity: 1, estimated_cost: '' }],
        payment_option: ucRequest.payment_option || 'PANJAR BIAYA 50%',
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('uc-requests.update', ucRequest.id), { preserveScroll: true });
    };
    ...
    return (
        <AuthenticatedLayout
            user={user}
            header={<h2 className="font-bold text-2xl text-gray-800 leading-tight">Edit Pengajuan Upcountry (UC)</h2>}
        >
            <Head title="Edit Pengajuan UC" />
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="mb-6 border-b border-gray-50 pb-4">
                        <h3 className="font-bold text-xl text-gray-800 flex items-center gap-2">
                            <PlaneTakeoff className="text-blue-600" /> Form Edit Biaya (UC)
                        </h3>
                        <p className="text-gray-500 text-sm mt-1">Lengkapi formulir di bawah ini untuk mengubah biaya perjalanan dinas Upcountry.</p>
                    </div>
