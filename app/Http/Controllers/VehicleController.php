...
class VehicleController extends Controller
{
    ...
    public function exportPdf()
    {
        ...
        if ($items->isEmpty()) {
            ...
            $allowed = ['license_plate', 'brand_type', 'manufacture_year', 'capacity', 'annual_tax_date'];
        ...
        $pdf = Pdf::loadView('pdf.generic_table', ['title' => 'Data Armada', 'headings' => $headings, 'rows' => $rows])->setPaper(request()->query('paper') === 'f4' ? [0, 0, 609.4488, 935.433] : request()->query('paper', 'a4'), request()->query('orientation', 'landscape'));
        return request()->has('preview') ? $pdf->stream(str_replace(' ', '_', 'Data Armada') . '.pdf') : $pdf->download(str_replace(' ', '_', 'Data Armada') . '.pdf');
    }
    ...
    public function exportExcel()
    {
        ...
        return request()->has('preview') ? response(\App\Helpers\ExcelPreviewHelper::render(new GenericExport($rows, $headings)))->header('Content-Type', 'text/html') : \Maatwebsite\Excel\Facades\Excel::download(new GenericExport($rows, $headings), str_replace(' ', '_', 'Data Armada') . '.xlsx');
    }
}
