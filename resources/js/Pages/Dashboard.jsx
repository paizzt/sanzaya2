    ...
    return (
        ...
        >
            ...
            <div className="overflow-hidden flex flex-col justify-center">
                {title && <p className="text-sm font-semibold text-gray-500 truncate">{title}</p>}
...
export default function Dashboard({ auth, stats, isAdmin }) {
    ...
    const getWidgetConfig = (key) => {
        const configs = {
            ...
            'Dashboard': { title: '', value: 'Dashboard', icon: LayoutDashboard, color: 'gray', action: () => router.visit(route('dashboard')) },
            'Ambil Absensi': { title: '', value: 'Ambil Absen', icon: Camera, color: 'blue', action: () => router.visit(route('absensi.index')) },
            'Izin/Sakit': { title: '', value: 'Izin/Sakit', icon: FileText, color: 'rose', action: () => router.visit(route('absensi.pengajuan')) },
            'Form Marketing': { title: '', value: 'Form Sales', icon: Briefcase, color: 'blue', action: () => router.visit(route('marketing.index')) },
            'Rekap Marketing': { title: '', value: 'Rekap Sales', icon: FileSpreadsheet, color: 'indigo', action: () => router.visit(route('marketing.recap.index')) },
            'Cari Produk': { title: '', value: 'Cari Produk', icon: Search, color: 'teal', action: () => router.visit(route('marketing.products.index')) },
            'Form UC': { title: '', value: 'Form UC', icon: CreditCard, color: 'emerald', action: () => router.visit(route('requests.uc.index')) },
            'Riwayat UC': { title: '', value: 'Riwayat UC', icon: FileClock, color: 'emerald', action: () => router.visit(route('requests.uc.history')) },
            'Persetujuan UC': { title: '', value: 'Persetujuan UC', icon: CheckCircle, color: 'emerald', action: () => router.visit(route('requests.uc.approval.index')) },
            'Input BHP': { title: '', value: 'Input BHP', icon: ClipboardList, color: 'orange', action: () => router.visit(route('requests.bhp.index')) },
            'Rekap BHP': { title: '', value: 'Rekap BHP', icon: FileSpreadsheet, color: 'orange', action: () => router.visit(route('requests.bhp.recap.index')) },
            'Pengajuan Pembayaran': { title: '', value: 'Buat Tagihan', icon: FileSignature, color: 'red', action: () => router.visit(route('payment-requests.index')) },
            'Data Hutang': { title: '', value: 'Data Hutang', icon: FileText, color: 'red', action: () => router.visit(route('payables.index')) },
            'Data Piutang': { title: '', value: 'Data Piutang', icon: FileText, color: 'cyan', action: () => router.visit(route('receivables.index')) },
            'Persetujuan Pembayaran': { title: '', value: 'Persetujuan Tagihan', icon: CheckCircle, color: 'red', action: () => router.visit(route('payment-approvals.index')) },
            'Laporan Logistik': { title: '', value: 'Lap Logistik', icon: Archive, color: 'yellow', action: () => router.visit(route('logistic-reports.index')) },
            'Surat Pesanan': { title: '', value: 'Surat Pesanan', icon: FileText, color: 'lime', action: () => router.visit(route('purchase-orders.index')) },
            'Data Penyedia': { title: '', value: 'Data Penyedia', icon: Building, color: 'sky', action: () => router.visit(route('providers.index')) },
            'Data Produk': { title: '', value: 'Data Produk', icon: Package, color: 'teal', action: () => router.visit(route('products.index')) },
            'Kebutuhan Barang': { title: '', value: 'Kebutuhan Brg', icon: Box, color: 'amber', action: () => router.visit(route('item-requirements.index')) },
            'Data Outlet': { title: '', value: 'Data Outlet', icon: Building, color: 'indigo', action: () => router.visit(route('outlets.index')) },
            'Pemetaan Outlet': { title: '', value: 'Pemetaan', icon: Map, color: 'indigo', action: () => router.visit(route('outlet-mappings.index')) },
            'Data Armada': { title: '', value: 'Data Armada', icon: Truck, color: 'slate', action: () => router.visit(route('vehicles.index')) },
            'Data Perusahaan': { title: '', value: 'Perusahaan', icon: Building, color: 'fuchsia', action: () => router.visit(route('company.index')) },
            'Data Pengguna': { title: '', value: 'Pengguna', icon: Users, color: 'blue', action: () => router.visit(route('users.index')) },
            'Manajemen SOP': { title: '', value: 'SOP', icon: ShieldCheck, color: 'indigo', action: () => router.visit(route('sops.index')) },
            'Rekap Absensi': { title: '', value: 'Rekap Absensi', icon: FileSpreadsheet, color: 'blue', action: () => router.visit(route('absensi.rekap')) },
            'Riwayat Perubahan': { title: '', value: 'Riwayat Log', icon: FileClock, color: 'gray', action: () => router.visit(route('system.activity-logs')) },
            'Notifikasi': { title: '', value: 'Notifikasi', icon: Bell, color: 'indigo', action: () => router.visit(route('notifications.index')) },
            'Profil & Akun': { title: '', value: 'Profil Akun', icon: User, color: 'gray', action: () => router.visit(route('profile.edit')) },
            'Stok Gudang': { title: '', value: 'Stok Gudang', icon: Box, color: 'amber', action: () => router.visit(route('warehouse-stocks.index')) },
            'Penggunaan Kendaraan': { title: '', value: 'Penggunaan Kdrn', icon: Truck, color: 'slate', action: () => router.visit(route('vehicle-usages.index')) },
            'Dashboard Laporan': { title: '', value: 'Dash Laporan', icon: LayoutDashboard, color: 'emerald', action: () => router.visit(route('reports.index')) },
            'Data Laporan Tersinkronisasi': { title: '', value: 'Data Laporan', icon: FileSpreadsheet, color: 'emerald', action: () => router.visit(route('spreadsheet.index')) },
        };
