...
class MarketingDailyReportController extends Controller
{
    ...
    public function exportPdf()
    {
        ...
        $pdf = Pdf::loadView('pdf.generic_table', ['title' => 'Laporan Marketing', 'headings' => $headings, 'rows' => $rows])->setPaper(request()->query('paper') === 'f4' ? [0, 0, 609.4488, 935.433] : request()->query('paper', 'a4'), request()->query('orientation', 'portrait'));
    ...
    public function exportTargetPdf()
    {
        ...
        $pdf = Pdf::loadView('pdf.generic_table', ['title' => 'Target Mingguan Marketing', 'headings' => $headings, 'rows' => $rows])->setPaper(request()->query('paper') === 'f4' ? [0, 0, 609.4488, 935.433] : request()->query('paper', 'a4'), request()->query('orientation', 'portrait'));
